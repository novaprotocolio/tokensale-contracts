...
contract('cloneNovaToken', ([fund, buyer, buyer2, wallet]) => {
  let tokenSale
  let novaToken
  let novaTokenFactory
  let novaTokenFactoryAddress
  ...
  beforeEach(async function() {
    ...
    novaTokenFactory = await TokenFactory.new()
    novaTokenFactoryAddress = await getAddress(novaTokenFactory)

    novaToken = await NovaToken.new(
      novaTokenFactoryAddress,
      '0x0',
      0,
      'Nova Token',
      'PRFT'
  ...
  describe('Cloning: ', function () {
    ...
    it('cloned token should return identical balances', async function() {
      let balance = await getTokenBalance(novaToken, buyer)
