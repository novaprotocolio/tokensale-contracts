...
contract('Crowdsale', accounts => {
  ...
  describe('Starting and Ending Period', async function() {
    ...
    it('should reject payments before start', async function() {
      ...
      await expectInvalidOpcode(
        tokenSale.buyTokens(sender, { value: 1 * ether, from: sender })
      );
    ...
    it('should accepts payments after start', async function() {
      ...
      await tokenSale.buyTokens(sender, { value: 1 * ether, from: sender })
        .should.be.fulfilled;
    });
  ...
  describe('Payments', async function() {
    ...
    it('should increase total token supply', async function() {
      ...
      let expectedSupplyIncrease = await numberOfTokensFor(
        tokenSale,
        1 * ether
      );
      expectedSupplyIncrease = await baseUnits(
        novaToken,
    ...
    it('should create tokens for the sender', async function() {
      ...
      let balanceIncrease = await baseUnits(
        novaToken,
        tokenBalance - initialTokenBalance
      );

      let expectedBalanceIncrease = await numberOfTokensFor(
        tokenSale,
        1 * ether
      );
      expectedBalanceIncrease = await baseUnits(
        novaToken,
  ...
  describe('Buying Tokens', async function() {
    ...
    it('should offer 12.62 tokens for 1 ether invested if less than 15% of the tokens were sold', async function() {
      ...
      let balanceIncrease = tokenBalance - initialTokenBalance;
      balanceIncrease = await baseUnits(novaToken, balanceIncrease);
      let expectedBalanceIncrease = 11.961722488;
      expect(balanceIncrease).almost.equal(expectedBalanceIncrease);
    });
